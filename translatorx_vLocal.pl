...
# Make a file that will send instructions to Clustal with appropriate name of input file
	...
	if ($pauser eq "clustalw") {
		...
		$muscleoutfile=$out_file . '.aa_ali.fasta';	
		print STDERR "/usr/local/bin/mafft --maxiterate 1000 --localpair --adjustdirectionaccurately $muscleinfile > $muscleoutfile\n";
		open(LOG, ">$out_file.$pauser.log") || die "Cannot create file outfile.pauser.log: $out_file.$pauser.log\n";
		my $pid = open(PH, "/usr/local/bin/mafft --maxiterate 1000 --localpair --adjustdirectionaccurately $muscleinfile 2>&1 1>$muscleoutfile |");              # or with an open pipe
		while(<PH>) {
			print STDERR $_;
			print LOG $_;
		}
